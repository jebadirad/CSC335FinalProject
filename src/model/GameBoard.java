...
public class GameBoard extends JFrame implements Serializable
{
  ...
  public void createRandomMap()
  {
    ...
    else
    {
      ...
      while (numberOfTerriansOnTheBoard < 150)
      {
        ...
        // Does nothing, no new Terrain assigned:
        else if (randomTerrain == 4 || randomTerrain == 5 || randomTerrain == 6)
        {
  ...
  /**
   * Generates two random units for player1, not standing in any type of terrain
   */
  public void generateRandomPlayer1Units()
  {
    ...
    while (numberOfUnits < 2)
    {
      ...
      if (random == 1)
      {
        ...
        // There is no terrain here:
        // Create a Clone Trooper
        Unit aUnit = factory.makeUnit("LukeSkywalker", GUI.getPlayer1());
        board[randomX][randomY].setUnit(aUnit);
        board[randomX][randomY].setHasUnit(true);
        // Add to player1Units:
        player1Units.add(board[randomX][randomY]);
    ...
    else
    {
      ...
      if (direction.equals("N"))
      {
            ...
            .getLocation().y].getTerrain() == Terrain.Boulder)
        {
          ...
          JOptionPane optionPane = new JOptionPane();
          optionPane.setMessage("You can't move into a Boulder, fool");
          JDialog dialog = optionPane.createDialog(":~(");
          dialog.setAlwaysOnTop(true);
          dialog.setVisible(true);
          result = false;
            ...
            .getLocation().y].hasUnit() == true)
        {
          ...
          JOptionPane optionPane = new JOptionPane();
          optionPane.setMessage("You can't move into another Unit, fool");
          JDialog dialog = optionPane.createDialog(":~(");
          dialog.setAlwaysOnTop(true);
          dialog.setVisible(true);
          result = false;
      ...
      else if (direction.equals("L"))
      {
        ...
        // Trying to move into a Boulder, return false.
        else if (board[cellWithUnit.getLocation().x][cellWithUnit.getLocation().y - 1]
            .getTerrain() == Terrain.Boulder)
        {
          // Tell them that they can't move into a boulder:
          JOptionPane optionPane = new JOptionPane();
          optionPane.setMessage("You can't move into a Boulder, fool");
  ...
  /**
   * This method moves the unit to a new cell, and returns the new cell with the
   * unit in it It check for terrain that reduces health or movement, or both
   * 
   * @param cellWithUnit
   *          This is given to me from the GUI contains a unit
   * @param direction
  ...
  public Cell move(Cell cellWithUnit, String direction)
  {
    if (direction.equals("N"))
    {
      ...
      if (t == Terrain.Lava)
      {
        ...
        if (board[cellWithUnit.getLocation().x][cellWithUnit.getLocation().y]
            .getUnit().getHealth() <= 0)
        {
          // Unit has died:
          board[cellWithUnit.getLocation().x][cellWithUnit.getLocation().y]
    ...
    for (int i = cellWithUnit.getLocation().x - 1; i >= 0; i--)
    {
      ...
      if (i == 0)
      {
            ...
            || board[i][cellWithUnit.getLocation().y].hasUnit() == true)
        {
          ...
          board[i + 1][cellWithUnit.getLocation().y].setUnit(cellWithUnit
              .getUnit());
        ...
        else
        {
          ...
          if (board[i][cellWithUnit.getLocation().y].getTerrain() == Terrain.QuickSand)
          {
            ...
            JOptionPane optionPane = new JOptionPane();
            optionPane.setMessage("Your Unit "
                + board[i][cellWithUnit.getLocation().y].getUnit().toString()
                + " has 4 movement due to Quicksand, fool");
            JDialog dialog = optionPane.createDialog(":~(");
            dialog.setAlwaysOnTop(true);
            dialog.setVisible(true);
          ...
          if (board[i][cellWithUnit.getLocation().y].getTerrain() == Terrain.Lava)
          {
            ...
            // Check see if unit has died:
            if (board[i][cellWithUnit.getLocation().y].getUnit().getHealth() <= 0)
            {
              // Unit has died:
              board[i][cellWithUnit.getLocation().y].getUnit().setMovesLeft(0);
              board[i][cellWithUnit.getLocation().y].getUnit().setCanAttack(
                  false);
              // Check which player owned the unit:
              optionPane.setMessage("Your Unit "
                  + board[i][cellWithUnit.getLocation().y].getUnit().toString()
              ...
              // Will always be player1Units since this is in the move
              // method:
              player1Units.remove(board[i][cellWithUnit.getLocation().y]);
    ...
    for (int i = cellWithUnit.getLocation().y + 1; i < 20; i++)
    {
      ...
      if (i == 19)
      {
            ...
            || board[cellWithUnit.getLocation().x][i].hasUnit() == true)
        {
          ...
          board[cellWithUnit.getLocation().x][i - 1].setUnit(cellWithUnit
              .getUnit());
        ...
        else
        {
          ...
          if (board[cellWithUnit.getLocation().x][i].getTerrain() == Terrain.Lava)
          {
            ...
            if (board[cellWithUnit.getLocation().x][i].getUnit().getHealth() <= 0)
            {
              ...
              board[cellWithUnit.getLocation().x][i].getUnit().setCanAttack(
                  false);
              // Check which player owned the unit:
              optionPane.setMessage("Your Unit "
                  + board[cellWithUnit.getLocation().x][i].getUnit().toString()
              ...
              // Will always be player1Units since this is in the move
              // method:
              player1Units.remove(board[cellWithUnit.getLocation().x][i]);
  ...
  /**
   * This method returns a list of cells with units in it, that can be attacked
   * by a cell containing a unit given to it
   * 
   * @param cellWithUnit
  ...
  /**
   * Attack method for two cells containing units being given: returns the cell
   * of unitBeingAttacked to the GUI, that updates the unit info in that cell:
   * 
  ...
  public Cell attack(Cell cellWithUnitAtacking, Cell cellWithUnitBeingAttacked)
  {
    ...
    if (cellWithUnitAtacking.getUnit().toString().equals("M"))
    {
      ...
      optionPane.setMessage("Your Medic Healed "
          ...
          + cellWithUnitBeingAttacked.getUnit().getDamage() + " Health!");
      JDialog dialog = optionPane.createDialog(":)");
      dialog.setAlwaysOnTop(true);
      dialog.setVisible(true);
    ...
    else
    {
      ...
      if (cellWithUnitBeingAttacked.getUnit().getHealth() <= 0)
      {
            ...
            .equals(GUI.getPlayer1()))
        {
          ...
          JOptionPane optionPane = new JOptionPane();
          optionPane.setMessage("Your Unit "
              + cellWithUnitBeingAttacked.getUnit().toString() + " Has Died!");
          JDialog dialog = optionPane.createDialog(":~(");
          dialog.setAlwaysOnTop(true);
          dialog.setVisible(true);
      ...
      else
      {
        ...
        optionPane.setMessage("Enemy Unit has "
            + cellWithUnitBeingAttacked.getUnit().getHealth() + " Health Left");
        JDialog dialog = optionPane.createDialog(":~D");
        dialog.setAlwaysOnTop(true);
        dialog.setVisible(true);
        return cellWithUnitBeingAttacked;
  ...
  /**
   * When the turn is over, update movesLeft, and setCanAttack, and change units
   * out:
   * 
   * @param player1units
   ...
   * @return null if there is no Unit in this cell, else returns the Unit in the
   *         cell
   */
  ...
  public void saveData(String p1, String p2)
  {
    try
    {
      FileOutputStream fileOut = new FileOutputStream(new File(saveDir + p1
          + "-" + p2 + "-" + "gameboard.dat"));
